    ...
    if (len == 0 || !($scope.msgs[len - 1].content.type == 'count-notice' && $scope.msgs[len - 1].content.count == cnt)) {
      ...
      if (type != '1') {
        ...
        if (cnt <= 0 || status == '0') {
          ...
          if (cnt == 3) {
            bodyDoc = '患者对您进行咨询，您最多需做三次回答，答满三次后，本次咨询结束；如不满三个问题，24小时后本次咨询关闭。请在24小时内回复患者。' + '\r\n' + '您还需要回答' + cnt + '个问题'
            bodyPat = '根据您提供的问题及描述，医生最多做三次回答，答满三次后，本次咨询结束，请尽量详细描述病情和需求；如不满三个问题，24小时后本次咨询关闭。医生会在24小时内回答，如超过24小时医生未作答，本次咨询关闭，且不收取费用。' + '\r\n' + '您还有' + cnt + '次提问机会'
          }
        }
      }
